...
最近发布：**V0.9.3 — Add-多行输入fix**（Git 标签 `v0.9.3`）。增加本地命令自动发现和终端多行输入，支持行号与直接粘贴多行文本。版本记录见 [CHANGELOG.md](CHANGELOG.md) 。
...
交互终端支持多行输入：回车只换行，连续两个空行后发送；续行前显示行号，直接粘贴多行文本时内部空行会保留。模型正在执行期间，权限确认 `y`／`n` 和本地命令仍按单行即时读取；管道输入继续使用原来的单行协议。
