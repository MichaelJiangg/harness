...
最近发布：**V1.0.4 — bugfix-上下文断档**（Git 标签 `v1.0.4`）。修复 Esc 中断后上下文未保存的问题。版本记录见 [CHANGELOG.md](CHANGELOG.md) 。
...
交互终端中，普通单行输入按一次回车即可发送。需要手工输入多行时，在第一行末尾加 `\` 后回车进入多行模式；直接粘贴多行文本会通过后续输入缓冲区自动识别，不会在第一行提前发送。多行模式内回车换行，连续两个空行后发送，续行前显示行号，内部空行会保留。空闲输入使用系统 readline 编辑，方向键移动、退格和修改均正常，提示符不会被误删。模型正在执行期间，权限确认 `y`／`n` 和本地命令仍按单行即时读取；按 `Esc` 可中断当前回答或正在执行的工具。管道输入继续使用原来的单行协议。
...
交互终端使用 `rich` 渲染：AI 回答以 Markdown 面板显示，支持标题、粗体、斜体、列表、链接和代码块语法高亮；用户输入、系统提示、错误、后台任务、委托和 Swarm 状态使用不同颜色。管道和非交互输出继续使用无 ANSI 的纯文本，方便脚本处理。

Harness 默认强制启用交互终端颜色；管道输出不会混入终端控制序列。

等待模型响应时显示 Rich 旋转动画和「思考中...」提示；收到流式文字后切换到逐字显示，交互终端默认每字约 20 毫秒，管道输出不限速。主查询直接执行工具时会显示工具名、参数快照、成功／失败状态和耗时；参数和结果会脱敏并按长度截断，不会打印文件正文。

逐请求用量、上下文压缩和编排内部事件继续进入会话内活动日志；输入 `/activity` 可展开查看工具参数、结果摘要、用量、压缩、重试和编排事件。权限确认和错误仍直接显示。
...
在同一会话中用 `/model` 切换 provider 后，`/cost` 会按 USD、CNY 等币种分组显示费用小计，不把不同币种直接相加；token 数仍统一汇总。
...
| `harness/commands/` | 自动发现、注册本地斜杠命令并生成 `/help` 列表 |
...
测试覆盖纯文字回答、多工具与多轮调用、委托与子查询、后台任务排队与跨轮查询、Swarm 多角色交接与回退、受限验证脚本、临时文件读写及错误恢复、关键词与文件类型搜索、行号及完整条目截断、写入和命令预览与逐次确认、拒绝和取消、命令两路输出与退出码、超时及同组进程清理、路径与软链接限制、连续对话、压缩与回滚、工具结果限长、递增重试与取消、限速显示、请求上限、token 和峰谷费用、多行输入与粘贴、`/clear`、`/history`、`/model`、`/tools`、`/cost`、`/compact`、启动及 `.env` 加载行为。文件和命令测试使用临时目录及无害本地子进程，接口与配置测试使用模拟内容，不读取实际密钥、不产生真实 API 费用。真实 API 联调尚未验证。
