...
当前进行中的改动为模型级热切换与终端输入修复：`/model` 直接列出并切换 `deepseek-flash`、`deepseek-pro`、`glm-5.3-flash`、`glm-5.3-pro`，其中 `deepseek-pro` 发送 API 模型名 `deepseek-v4-pro`；busy 输入关闭回显并由程序手动回显，流式渲染清洗控制字符。V1.0.3 已发布。

最近公开版本为 [V1.0.3 — bugfix-乱码修复](https://github.com/MichaelJiangg/harness/releases/tag/v1.0.3) ，仓库为 [MichaelJiangg/harness](https://github.com/MichaelJiangg/harness) ，标签为 `v1.0.3`。
