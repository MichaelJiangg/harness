...
当前进行中的改动为模型级热切换：`/model` 直接列出并切换 `deepseek-flash`、`deepseek-pro`、`glm-5.3-flash`、`glm-5.3-pro`，其中 `deepseek-pro` 发送 API 模型名 `deepseek-v4-pro`；`--model` 自动推断 provider 并解析别名。V1.0.2 已发布。

最近公开版本为 [V1.0.2 — bugfix-模型切换](https://github.com/MichaelJiangg/harness/releases/tag/v1.0.2) ，仓库为 [MichaelJiangg/harness](https://github.com/MichaelJiangg/harness) ，标签为 `v1.0.2`。
